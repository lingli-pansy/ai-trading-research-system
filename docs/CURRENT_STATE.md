...
- **Paper**：`python cli.py paper [--symbol SYMBOL] [--once] [--mock] [--llm]`，Research → Contract → 本仓 Paper 注入（或配置 IBKR_* 后走 TWS）。
...
- **暂缓（需授权后对接）**：IBKR Paper 真实下单、生产 API Key、OpenClaw 服务端对接；见 [deferred_authorization.md](deferred_authorization.md)。本仓 Paper 与 OpenClaw 报告已可用，不阻塞 MVP。
...
- 本仓 **RuleEngine**、**PortfolioEngine**、**PaperTradingEngine**（当前执行路径）。
- 当前 **backtest runner**（`backtest/runner.py` 等），回测由 Nautilus 驱动但执行层仍为本仓封装。
- 上述模块在 [restructuring_plan.md](restructuring_plan.md) 的 Phase 1–5 中将被 NautilusTrader 统一回测与实盘、Strategy Bridge、Experience 闭环逐步替换。
...
- **授权就绪后**：IBKR Paper、OpenClaw 服务端、生产密钥、风控对接步骤见 [deferred_authorization.md](deferred_authorization.md)。
