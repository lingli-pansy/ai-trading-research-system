...
目标架构见 [docs/architecture/system_architecture_overview.md](docs/architecture/system_architecture_overview.md)。

## 快速开始

### 首屏试用一条主线（按顺序执行）

1. **环境**：`pip install -e .`（可选：venv、`cp .env.example .env`，见 [docs/integration/cli_usage.md](docs/integration/cli_usage.md)）。
...
5. **OpenClaw 调用示例**：`python cli.py research NVDA --mock` 或 `python scripts/run_for_openclaw.py research NVDA --mock`，stdout 为单条 JSON；完整报告见 [docs/integration/openclaw_integration.md](docs/integration/openclaw_integration.md)。
...
**开发前准备**：运行 `python scripts/check_dev_prerequisites.py` 核对环境；详见 [docs/integration/cli_usage.md](docs/integration/cli_usage.md)。
...
**一句命令跑通**：`python cli.py demo NVDA` 可看到研究结论、策略生成、回测结果、交易总结四块输出。CLI 与 OpenClaw Skill 打通，见 [docs/integration/openclaw_integration.md](docs/integration/openclaw_integration.md)。
...
- **输出**：stdout 为单条 JSON（含 task、symbol、contract_action、contract_confidence、thesis_snippet、raw_contract 等）。完整字段见 [docs/integration/openclaw_integration.md](docs/integration/openclaw_integration.md)。
- **验证**：退出码 0 即成功；可 pipe 到 `jq` 或 Python 解析。Skill 调用方式与报告格式见 openclaw_integration。

## 脚本用法（兼容 / 高级）

前置：环境与依赖见 [docs/integration/cli_usage.md](docs/integration/cli_usage.md)；可选 `.env` 中 OPENAI_API_KEY 或 KIMI_CODE_API_KEY。上述 CLI 子命令与下列脚本对应，可按需选用。
...
**OpenClaw 集成**：Skill 可调用 `cli.py` 或 control 层；报告格式见 [docs/integration/openclaw_integration.md](docs/integration/openclaw_integration.md)。

数据与存储：行情默认 yfinance；Experience Store 默认 `.experience/experience.db`（可通过 `EXPERIENCE_DB_PATH` 覆盖）。回测历史写入临时目录 `.backtest_catalog`。

## 文档（开发入口）

所有文档在 `docs/` 下，**入口为 [docs/README.md](docs/README.md)**：Architecture、Core Concepts、Execution、Experience、Integrations、Operations；历史文档在 `docs/archive/`。

| 文档 | 说明 |
|------|------|
| [docs/README.md](docs/README.md) | **文档入口**（Architecture / Core Concepts / Execution / Experience / Integrations / Operations） |
| [docs/operations/current_state.md](docs/operations/current_state.md) | **当前状态总表**：能跑通什么、mock、过渡层、下一步 |
| [docs/architecture/system_architecture_overview.md](docs/architecture/system_architecture_overview.md) | 目标架构总览 |
| [docs/core_concepts/decision_contract.md](docs/core_concepts/decision_contract.md) | DecisionContract 规范 |
| [docs/core_concepts/strategy_spec.md](docs/core_concepts/strategy_spec.md) | StrategySpec 规范 |
| [docs/experience/experience_store.md](docs/experience/experience_store.md) | ExperienceStore 表与写入口 |
| [docs/archive/](docs/archive/) | 历史文档（restructuring_plan、mvp_plan、live_readiness_checklist、dev_prerequisites、PRD 等） |
...
├── docs/                  # 入口 docs/README.md；architecture/、core_concepts/、execution/、experience/、integration/、operations/、archive/
├── scripts/
├── src/
