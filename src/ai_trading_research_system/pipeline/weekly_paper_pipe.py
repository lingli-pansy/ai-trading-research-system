...
def _progress(msg: str) -> None:
    """打印进度到 stderr，便于长时间运行时有反馈。"""
    print(f"[weekly-paper] {msg}", file=sys.stderr, flush=True)
...
def run_weekly_autonomous_paper(
    *,
    ...
    _progress(f"Start: {len(symbols_list)} symbols, {duration_days} days, use_llm={use_llm}, use_mock={use_mock}.")

    # 根治连接稳定性：非 mock 且配置了 IB 时，整轮复用单连接（IBKRSession），避免频繁 connect/disconnect
    ib_session = None
    if not use_mock:
        try:
    ...
    try:
        ...
        _progress("Getting regime context (SPY/VIX)...")
        ...
        for day in range(duration_days):
            ...
            _progress(f"Day {day + 1}/{duration_days}: done (trades={day_trades}, pnl={day_pnl:.2f}).")
        ...
        if ib_session is not None:
            try:
                ...
                _progress("IB session disconnected.")
            except Exception:
                pass
