...
每次评估记录 trigger_trace（Decision Traceability）。
"""
...
from ai_trading_research_system.autonomous.decision_trace import TriggerTrace
...
def _health_excerpt(portfolio_health: Any) -> dict[str, Any]:
    if portfolio_health is None:
        return {}
    h = portfolio_health
    if isinstance(h, dict):
        return {k: h.get(k) for k in ("concentration_index", "beta_vs_spy", "max_drawdown", "volatility") if k in h}
    return {
        "concentration_index": getattr(h, "concentration_index", None),
        "beta_vs_spy": getattr(h, "beta_vs_spy", None),
        "max_drawdown": getattr(h, "max_drawdown", None),
        "volatility": getattr(h, "volatility", None),
    }
...
def evaluate_intraday_triggers(
    ...
    *,
...
) -> tuple[AdjustmentTrigger | None, TriggerTrace]:
    """
    评估是否应触发日内调整。返回 (第一个满足条件的 trigger 或 None, trigger_trace)。
    ...
    health_ctx = _health_excerpt(portfolio_health)
    ...
    for o in opportunity_ranking or []:
        if (o.get("risk") or "").lower() == "high":
            ...
            return t, TriggerTrace(timestamp=now, trigger_fired=True, trigger_type=t.trigger_type, trigger_reason=t.trigger_reason, health_context=health_ctx, severity=t.severity)
    ...
    if not opportunity_ranking:
        return None, TriggerTrace(timestamp=now, trigger_fired=False, health_context=health_ctx)
